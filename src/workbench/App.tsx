...
function Dossier({ client, run, stage, onBack, onRefresh }: { client: ApiClient; run: Run; stage: string; onBack: () => void; onRefresh: () => Promise<void> }) {
  ...
  return <section className="view dossier-view" aria-labelledby="dossier-title"><div className="breadcrumb"><button onClick={onBack}>Mission Control</button><span>/</span><button onClick={onBack}>Planning Relay</button><span>/</span><b>{statusLabel(stage)}</b></div><header className="flow-header"><div><div className="title-line"><h1 id="dossier-title">{statusLabel(stage)}</h1><Pill status={stage === run.active_gate ? "awaiting" : run.status} /></div><p className="mono">node_{stage.replaceAll("_", "-")} <span /> {stageType(stage)} <span /> Project {run.project_id}</p></div><div className="kpis"><Kpi label="Evidence" value={run.artifacts.length} /><Kpi label="Gate" value={run.active_gate ?? "—"} /><Kpi label="Status" value={statusLabel(run.status)} /></div></header><div className="tabs" role="tablist">{tabs.map((item) => <button key={item.id} role="tab" aria-selected={tab === item.id} onClick={() => setTab(item.id)}>{item.label}</button>)}</div>
    {tab === "overview" && <div className="dossier-grid"><section className="card"><p className="eyebrow">Authoritative stage</p><h2>{statusLabel(stage)}</h2><p>This stage is projected by Cogito from the persisted planning run. Decisions and evidence remain server-authoritative.</p><dl><dt>Project</dt><dd>{run.project_id}</dd><dt>Workflow state</dt><dd>{statusLabel(run.status)}</dd><dt>Active gate</dt><dd>{run.active_gate ?? "None"}</dd><dt>Submitted</dt><dd>{new Date(run.submitted_at).toLocaleString()}</dd></dl></section><section className="card"><p className="eyebrow">Verified evidence</p><div className="artifact-list">{run.artifacts.map((item) => <button key={item.kind} className={artifact?.kind === item.kind ? "selected" : ""} onClick={() => void openEvidence(item)}><b>{item.kind}</b><small>{item.sha256.slice(0, 12)}</small></button>)}</div>{error && <p role="alert">{error}</p>}{evidence && <pre aria-label="Verified evidence">{evidence}</pre>}</section></div>}
    ...
    {tab === "configuration" && <section className="card"><pre>{JSON.stringify({ run_id: run.run_id, project_id: run.project_id, stage, workflow: run.workflow, artifacts: run.artifacts.map((item) => ({ kind: item.kind, sha256: item.sha256 })) }, null, 2)}</pre></section>}
    {tab === "dependencies" && <section className="card dependency-list">{run.workflow.map((item) => <div key={item}><span><b>{statusLabel(item)}</b><small>{stageType(item)}</small></span><Pill status={item === run.active_gate || item.includes("approval") ? "awaiting" : run.status} /></div>)}</section>}
    {tab === "history" && <section className="card timeline"><p><i />Run submitted <small>{relativeTime(run.submitted_at)}</small></p><p><i />Current projection loaded <small>now</small></p></section>}
