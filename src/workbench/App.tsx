...
function ProductPlanSummary({ content, artifact }: { content: string; artifact: Artifact | null }) {
  ...
  if (!plan) return null;
  return <section className="card" aria-labelledby="verified-plan-summary-title"><p className="eyebrow">Verified plan summary</p><h3 id="verified-plan-summary-title" className="panel-title">{plan.title}</h3><p className="dossier-description">{plan.summary}</p><div className="timeline">{plan.phases.map((phase) => <div className="tl-item" key={phase.id}><i /><span><b>{phase.name}</b><small>{phase.description}</small>{phase.acceptanceCriteria.length > 0 && <small>Acceptance: {phase.acceptanceCriteria.join(" · ")}</small>}</span></div>)}</div></section>;
}
...
function EvidenceViewer({ client, run, initial, stageId }: { client: ApiClient; run: Run; initial?: Artifact["kind"]; stageId?: string }) {
  ...
  return <section className="card evidence-card"><h2 className="panel-title">Verified immutable evidence</h2><div className="artifact-list">{run.artifacts.map((artifact) => <button key={`${artifact.kind}:${artifact.sha256}`} className={selected?.sha256 === artifact.sha256 ? "selected" : ""} aria-expanded={selected?.sha256 === artifact.sha256 && evidenceExpanded} onClick={() => void open(artifact)}><b>{artifact.kind === "source" ? "Specification" : artifact.kind}</b><small>{artifact.sha256.slice(0, 12)}</small></button>)}</div>{selected && <p className="control-note">Digest: <span className="mono">{selected.sha256}</span></p>}{loading && <p className="control-note" role="status">Loading verified evidence…</p>}{error && <p className="evidence-error" role="alert">{error}</p>}{content && evidenceExpanded && <><ProductPlanSummary content={content} artifact={selected} /><pre aria-label="Verified evidence">{content}</pre></>}{stageId && selected && <section className="card review-context"><h3 className="panel-title">Review context</h3><p id="review-context-help" className="control-note">Capture immutable rationale for reviewers of this exact evidence. It does not instruct an agent or change execution; use Request revision when the work itself needs to change.</p><label className="form-field" htmlFor="review-context"><span>Context for reviewers</span><textarea id="review-context" className="form-textarea" aria-describedby="review-context-help review-context-limit" value={note} onChange={(event) => setNote(event.target.value)} maxLength={10_000} placeholder="Add decision context, assumptions, or rollout concerns…" /></label><p id="review-context-limit" className="form-help">Up to 10,000 characters. Recording is permanent for this stage and digest.</p><div className="form-actions"><button className="button-primary" disabled={!note.trim() || submittingFeedback} aria-busy={submittingFeedback} onClick={() => void submitNote()}>{submittingFeedback ? "Recording…" : "Record context"}</button></div>{loadingFeedback && <p className="control-note" role="status">Loading recorded review context…</p>}{feedbackNotice && <p className="sync-row" role="status">{feedbackNotice}</p>}{feedback && (visibleFeedback.length ? <div className="timeline" aria-label="Recorded review context">{visibleFeedback.map((item) => <div className="tl-item" key={item.feedback_id}><i /><span><b>{item.actor_id}</b><small>{new Date(item.created_at).toLocaleString()} · {item.comment}</small></span></div>)}</div> : <p className="control-note">No review context is bound to this stage and digest.</p>)}</section>}{!selected && <p className="control-note">No immutable evidence is available for this run.</p>}</section>;
}
...
function EmbeddedStageDossier({ client, run, node, edges, timeline, tab, setTab, onRefresh, decisionNotice, onDecisionComplete }: { client: ApiClient; run: Run; node: PositionedWorkflowNode; edges: WorkflowEdge[]; timeline: TimelineEvent[]; tab: NodeDossierTab; setTab: (tab: NodeDossierTab) => void; onRefresh: () => Promise<void>; decisionNotice: string | null; onDecisionComplete: () => void }) {
  ...
  return <section className="embedded-dossier" aria-labelledby="embedded-dossier-title"><header><div><p className="eyebrow">Selected-stage dossier</p><h2 id="embedded-dossier-title">{node.name}</h2></div></header><div className="subtabs" role="tablist" aria-label="Selected-stage dossier views">{tabs.map((item) => <button key={item.id} className="subtab" role="tab" aria-selected={tab === item.id} onClick={() => setTab(item.id)}>{item.label}</button>)}</div><div className="embedded-dossier-content">{decisionNotice && <p className="sync-row" role="status">{decisionNotice}</p>}{tab === "overview" && <div className="dossier-grid"><section className="card dossier-summary"><p className="eyebrow">Authoritative node context</p><p className="dossier-description">{node.reason}</p><div className="dossier-details"><div><span>Node type</span><b>{node.type}</b></div><div><span>State source</span><b>{node.availability}</b></div><div><span>Evidence</span><b>{node.artifactKind ?? "Unavailable"}</b></div><div><span>Workflow scope</span><b>{run.project_id}</b></div></div></section><section className="card dossier-events"><h3 className="panel-title">Recent events</h3>{events.length ? <div className="timeline">{events.map((event) => <div className="tl-item" key={event.event_id}><i /><span><b>{statusLabel(event.event_type)}</b><small>{new Date(event.occurred_at).toLocaleString()}</small></span></div>)}</div> : <p className="control-note">No persisted events are attributable to this node.</p>}</section>{node.id === `${run.active_gate}_approval` && <DecisionControls client={client} run={run} onComplete={onRefresh} onSuccess={onDecisionComplete} />}</div>}{tab === "audit" && <section className="card log-card"><h3 className="panel-title">Authoritative audit activity</h3><p className="control-note">This view contains durable lifecycle and approval events, not raw agent output.</p>{events.length ? events.map((event) => <p key={event.event_id}><span>{new Date(event.occurred_at).toLocaleTimeString()}</span> <b>[{event.gate ?? node.type}]</b> {statusLabel(event.event_type)}</p>) : <p className="control-note">No authoritative audit activity is available for this node.</p>}</section>}{tab === "specifications" && (node.artifactKind ? <EvidenceViewer client={client} run={run} initial={node.artifactKind} stageId={node.id} /> : <section className="card"><h3 className="panel-title">Verified specification and evidence</h3><p className="control-note">No immutable evidence is available for this node.</p></section>)}{tab === "configuration" && <section className="card"><h3 className="panel-title">Configuration availability</h3><p className="control-note">No persisted node configuration is available. This bounded display context is authoritative.</p><pre className="codeblock" aria-label="Authoritative node display context">{JSON.stringify({ node_id: node.id, type: node.type, state_source: node.availability, evidence_kind: node.artifactKind }, null, 2)}</pre></section>}{tab === "dependencies" && <section className="dependency-list">{dependencies.length ? dependencies.map((dependency) => <div key={dependency.id}><span><b>{dependency.name}</b><small>{dependency.type}</small></span><Pill status={dependency.status} /></div>) : <p className="control-note">This node has no persisted graph dependencies.</p>}</section>}{tab === "history" && <section className="card"><h3 className="panel-title">Persisted lifecycle history</h3>{events.length ? <div className="timeline">{events.map((event) => <div className="tl-item" key={event.event_id}><i /><span><b>{statusLabel(event.event_type)}</b><small>{new Date(event.occurred_at).toLocaleString()} · {event.delivered ? "delivered" : "pending delivery"}</small></span></div>)}</div> : <p className="control-note">No persisted lifecycle history is available for this node.</p>}</section>}</div></section>;
}
